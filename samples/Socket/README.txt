...
select indicate when the attempt would block; in those cases, the
stepper does not call send or recv, so no blocking occurs.  This
enables pmt to proceed and make another call that results in
...
You will also see that the socket connection blocks when the buffer is
full or empty.  When the (quite small) buffer is full, send_call is
not immediately followed by send_return.  Instead, recv_return must
execute first, in order to free some space in the buffer.  Likewise,
recv blocks when the buffer is empty, and cannot return until
send_return leaves a message in the buffer.  If you are using
stepper_a, you may notice pauses when these blocks occur.  If you are
using the synchronous stepper, you may see the entire pmt session
block and not resume.  The only escape from this is to type ^C.
